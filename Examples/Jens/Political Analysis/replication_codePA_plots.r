...
setwd("~/Dropbox/Atacama 2015/02_analysis/replication hainmueller/Political Analysis")
...
d <- GetLabels(d)
  ...
  dd <- data.frame(var= c("Gender:",
                          ...
                          "        ",
                          "Prior trips to U.S.:"
  ),order=c(.5,2.1,2.5,4.1,4.5,9.1,9.5,23.1,23.5,34.1,34.5,38.1,38.5,42.1,42.5,45.1,45.5),
                   pe=1,se=1,upper=1,lower=1,gruppe=NA)
  d <- rbind(d,dd)
  d <-d[order(d$order),]
  d$var <- factor(d$var,levels=unique(d$var)[length(d$var):1])
