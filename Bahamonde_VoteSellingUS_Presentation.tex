...
\miniframeson
\begin{frame}\frametitle{Vote Selling: Social Desirability Bias}
    \begin{itemize}
      \item Directly asking respondents whether they would sell their votes will cause {\bf social desirability bias}.
      \item Respondents might feel ashamed/embarrassed when admitting doing something socially condemnable.
...
\miniframesoff
\begin{frame}\frametitle{List Experiments: Overcoming Social Desirability Bias}
    \begin{itemize}
      ...
      \item {\bf Mechanics}: 
        ...
        \end{itemize}
      \item {\bf Easy estimation}: since both lists are assigned at random, any difference in means should be attributed to the sensitive item \emph{only}.
    \end{itemize}
\end{frame}
...
\begin{minipage}{0.4\textwidth}
    \begin{itemize}
          \item Sum of items each subject $i$ declares.
          \item Two treatments were administered (``cheap''/``expensive''): account for possible elasticities.\\{\scriptsize Hard to price a vote.}
    \end{itemize}
\end{minipage}
\begin{minipage}{0.5\textwidth}
