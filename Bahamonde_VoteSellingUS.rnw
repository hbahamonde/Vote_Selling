...
Having the test been based on the direct question its results require a word of caution. The list experiment does suggest that some respondents lied when directly asked if they would sell their votes. Consequently, we should expect the willingness to sell test to be biased in some degree. Also, only a small proportion of likely vote-sellers answered the direct question affirmatively. In addition to that, prices are product of supply and demands dynamics. In this context that means that prices result from the interaction between parties (buyers) and voters (sellers). In this design we just observe the sellers' side. Hence, we limit our inferences even more by thinking about these results as just suggestive of some willingness to sell. Hence, more than acting as definitive and final pricing tests, these findings do seem to suggest that the vote-selling price is high enough to deter political parties from engaging in vote-selling. Finally, future research is encouraged to elaborate more complex studies where supply and demands dynamics are incorporated into the design.\todo{fixed but double check} 
