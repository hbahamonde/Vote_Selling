...
While this paper essentially describes the phenomenon, it leaves for future research further considerations about the causes of hypothetical vote-selling in the United States. Ultimately, this paper is an attempt to bring voters back in the quantitative study of clientelism, particularly, vote-selling. 
...
\autoref{tab:t:test} \hypertarget{statistical:analyses}{shows} a simple difference in means analysis between each treated group and the control group. On average, the control group would do $\Sexpr{as.character(as.numeric(round(dif.means.control$par.control, 3)))}$ on the list. Subjects treated under the ``low'' condition (\$100) would do $\Sexpr{as.character(as.numeric(round(dif.means.low$par.treat + dif.means.low$par.control, 3)))}$ on the list, while subjects in the ``high'' condition (\$500) would do $\Sexpr{as.character(as.numeric(round(dif.means.high$par.treat + dif.means.high$par.control, 3)))}$ things. 
...
\hline
\end{tabular}
\caption{{\bf Differences in Means between Treatments (high and low) and the Control Group.\label{tab:t:test}}\\\hspace{\textwidth} {\bf Note}: \autoref{tab:t:test} shows two-tailed t-tests between each experimental treated unit (``high'' and ``low'' conditions) and the control group. The table shows that \Sexpr{as.character(as.numeric(round(dif.means.high.predicted$fit["fit"], 3)))} $\times\;100\;=\;$ \Sexpr{as.numeric(round(dif.means.high.predicted$fit["fit"], 3))*100}\% of subjects would sell their votes under the ``high'' condition, while \Sexpr{as.character(as.numeric(round(dif.means.low.predicted$fit["fit"], 3)))} $\times\;100\;=\;$ \Sexpr{as.numeric(round(dif.means.low.predicted$fit["fit"], 3))*100}\% of subjects would sell their votes under the ``low'' condition. Also, 95\% confidence intervals are shown. It is evident they are quite wide and are not statistically significant.}
\centering
\end{table}



There are three important points regarding this bivariate analysis. First, the mean differences between treated groups (``high'' and ``low'' treatments) is statistically zero. That implies that neither treatment should introduce design bias into the experiment. Second, while treated subjects do have slightly higher means when compared to the control group (i.e., indicating some propensity to sell their votes), these differences are not statistically significant. Third, while not statistically significant, $\Sexpr{as.character(as.numeric(round(dif.means.high.predicted$fit["fit"], 3)))}$ $\times\;100\;=\;$ $\Sexpr{as.numeric(round(dif.means.high.predicted$fit["fit"], 3))*100}$\% of subjects would sell their votes under the ``high'' condition, while $\Sexpr{as.character(as.numeric(round(dif.means.low.predicted$fit["fit"], 3)))}$ $\times\;100\;=\;$ $\Sexpr{as.numeric(round(dif.means.low.predicted$fit["fit"], 3))*100}$\% of subjects would sell their votes under the ``low'' condition. As shown below, these figures score way under what is found in this experimental design.
