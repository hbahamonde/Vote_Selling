...
writeLines(paste("In nineteenth-century United States politics, vote-buying was commonplace. Nowadays, vote-buying seems to have declined. Yet, the quantitative empirical literature emphasizes  vote-buying, ignoring the micro-dynamics of vote-selling. We seem to know that vote-buyers can no longer afford this strategy; however, we do not know what American voters would do if offered the chance to sell their vote. Would they sell it (and at what price), or would they consistently opt out of vote-selling? Exploiting a novel experimental dataset representative at the national level, ", total.sample.size, "U.S. voters participated in an online list experiment in 2016, and the results are striking: Approximately ", paste(round(((socdes.p.high.low.diff.in.means$fit[socdes.p.high.low.diff.in.means$c.1.3=="List\nExperiment\n(with covariates)"][1] + socdes.p.high.low.diff.in.means$fit[socdes.p.high.low.diff.in.means$c.1.3=="List\nExperiment\n(with covariates)"][2]) / 2) * 100, 0), "\\%", sep=""), "would sell their vote for a minumum payment of ", paste("\\$", round(mean(pricing.d$value[pricing.d$variable=="Cheap"]),0), ".", sep=""), "Democrats and Liberals are systematically more likely to sell, while education or income levels do not seem to impact the likelihood of vote-selling."), fileConn)
close(fileConn)
## ----
